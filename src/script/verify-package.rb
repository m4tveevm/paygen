...
abort 'Packaged license differs from repository license' unless File.binread(File.join(root, 'LICENSE')) == File.binread(File.join(root, 'src/LICENSE'))
...
Dir.mktmpdir('paygen-package-') do |directory|
  ...
  run.call({}, File.join(root, 'src'), '-S', 'gem', 'build', 'paygen.gemspec', '--output', archive)
  package = Gem::Package.new(archive)
  required = %w[bin/paygen lib/paygen.rb lib/paygen/runtime/demo/index.html
                lib/paygen/core/schemas/arazzo-1.1.json recipes/novapay.yml]
