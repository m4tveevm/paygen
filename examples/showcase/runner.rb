...
module PaygenShowcase
  class Runner
    ...
    def adaptation
      ...
      cli('adapt-stale-generation', 'generate', adapted, expected: 4)
      check('adapt:changed_contract_needs_review', File.read(File.join(@output, 'adapt-stale-generation.stderr')).include?('REVIEW_STALE'))
      cli('adapt-profile', 'configure', adapted, '--answers', File.join(adapted, 'integration.yml'), '--set', 'amount.minimum=200000')
