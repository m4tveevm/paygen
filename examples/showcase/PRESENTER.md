...
| 2:00–3:30 | Start `bundle exec src/bin/paygen demo tmp/live/novapay --port 9293`, then open its panel | Server response contains provider ID; evidence commit delta is one | [PROJECT POLICY] generated adapter really executes. Use saved `novapay-run-*-*.json`; do not interact during the automated run. |
