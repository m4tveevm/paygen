...
**Статус исследования:** R0–R3 READY_FOR_REVIEW; результаты относятся к commit `92ef59bc2c8eb102c7452136ee4ea8a46887fd52` (6 сентября 2026 года). Финальное подтверждение release candidate не выполнялось: `FUNCTIONAL_FREEZE`/`RELEASE_CANDIDATE` и `RELEASE_CONTRACT` не переданы.
...
На исследованном baseline реализован конвейер Input → Overlay → IR/profile/provenance → Generator → Adapter/Simulator/Verifier/StateFuzzer. Полный локальный RSpec-прогон дал 1119 примеров без отказов при seed 1016; 703 примера относятся к JSONPath CTS, поэтому число 1119 нельзя выдавать за число платёжных тестов. Покрытие в этом прогоне — 89,46% строк и 74,76% ветвей. Эти числа доказывают только поведение данного checkout и набора тестов; они не подтверждают совместимость с закрытым backend, реальный provider sandbox, settlement, PCI DSS compliance или exactly-once между процессами.
...
| RQ3 | Возможен ли отличающийся provider без core patch? | Onboarding native PayPal/Paystack на фиксированных snapshots и profiles; `git diff lib/`. | Executable profiles, core diff, unsupported diagnostics. | Не доказывает произвольный provider и production readiness. |
...
## 6. Наблюдаемые результаты baseline

Единственный завершённый количественный эксперимент этой фазы — полный `bundle exec rspec` на указанном SHA, Linux x86_64, Ruby 3.4.4, seed 1016, WebMock с разрешённым localhost. Результат: **1119 examples, 0 failures**, 96,69 s внутри RSpec; line coverage 4098/4581 = 89,46%, branch coverage 1884/2520 = 74,76%. Raw log и exit code находятся в `research/evidence/`. Из 1119 примеров **703 — JSONPath CTS** согласно baseline breakdown, поэтому они проверяют selector engine, а не 703 платёжных сценария.
...
Эксперименты byte-identical regeneration, semantic YAML/JSON equivalence, drift/preservation, configuration-only onboarding, matched-budget mutations, shrink/replay и cold/warm timings в этой фазе **NOT_RUN**: отсутствует release candidate, а перенос baseline-чисел на будущий SHA нарушил бы трассируемость. В `EXPERIMENTS.md` записаны команды и критерии. Пять известных baseline-дефектов A1–A5 также не объявлены исправленными: credential-less cache scope, late workflow output dependency, callback dedupe по нормализованному status, смена `BigDecimal` на `String` после cache copy и ложный duplicate ref ID при возврате к root filename требуют отдельных regression artifacts на final SHA.
...
Практический workflow: pin source и hash → inspect diagnostics → применить reviewable Overlay → заполнить semantic profile без догадок → generate → проверить drift → исполнить adapter через loopback simulator → прогнать independent vectors, faults и replay → вручную согласовать backend hooks → проверить provider sandbox → только затем проектировать production storage, observability и compliance. Следующие обязательные шаги: получить `RELEASE_CONTRACT` и release SHA; повторить все эксперименты из приложения; добавить regression evidence A1–A5; провести independent wire review NovaPay (обе ветви recipient); проверить адаптацию raw-body callback hook к host. До этого статус результатов — **BLOCKED_DEPENDENCY**, а не DONE.
